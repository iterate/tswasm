...
export async function createBrowserRpcFixture<TInstance extends object>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "tswasm-browser-rpc-"));
      ...
      return async (...args: unknown[]) => {
        ...
        try {
          ...
          await page.waitForFunction(
            ({ requestId }) => {
              const renderedRequestId = document.querySelector(
                "[data-testid=\"rpc-request-id\"]",
              )?.textContent;
              const renderedStatus = document.querySelector("[data-testid=\"rpc-status\"]")?.textContent;
...
async function waitForFixtureBoot(
  ...
  try {
    await page.waitForFunction(
      () => {
        const status = document.querySelector("[data-testid=\"boot-status\"]")?.textContent;
