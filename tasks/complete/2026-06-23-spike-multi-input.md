...
Status summary: Complete. The spike adds a backward-compatible virtual project API, resolves relative imports across in-memory files, returns emitted JavaScript by output file name, parses virtual `tsconfig` files, supports virtual `cwd`/`typeRoots`, documents the behavior, and passes the local release check.
