...
Follow-up adjustment: emit-only tables now repeat `tswasm warm compile` and
`TypeScript JS full program (in-memory)` as reference rows. Native curiosity
tables repeat `tswasm warm compile` and `tswasm cold createCompiler+compile`.
Those comparisons are not the main decision criteria, but showing the reference
rows makes the magnitude visible without forcing readers to jump between
tables.

The CLI tables are generated by `bench/compile.bench.ts`; the README tables are
static representative output. A duplicate `fastest` label in the type-heavy
native curiosity README table exposed two issues: the README row order was
stale, and the generator used a near-tie threshold that could label more than
one row `fastest`. The generator now labels only the sorted minimum row as
`fastest`; near ties are reported as `within 0.5%`.
